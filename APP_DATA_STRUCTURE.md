...
| 市区町村検索数 | 1,896件 |
| 地図フィーチャ総数 | 721,269件 |
| アプリ一式の容量 | 約289 MiB（303,329,992 bytes、作業用ZIP等を除く） |
...
| 市区町村別データ | `data/by_municipality/{municipality_code}.geojson` | 1,896ファイル / 約246 MiB | 選択した自治体のピン、面、件数、歴史・風土コメント | 市区町村選択時に1ファイルだけ | 必須 |
...
7. Pointが多い自治体では、地図描画用Pointをカテゴリ比率に応じた最大3,500件の代表表示に抑える。結果件数は `layer_counts` / `total_features` の全件数を使う。
8. 最大200本のPointだけを、X座標を変えず画面外上部から下方向へ落とす。
9. `layer_counts`、`total_features`、`display_comment` を結果画面へ表示する。

全国約258 MiBのランタイムデータを一度に通信するわけではありません。通常の1回の検索では、検索リストと選択自治体のGeoJSON・境界だけを読みます。
