...
function updateMapData(data) {
  const byLayer = groupFeaturesByLayer(data.features);
...
function showResult(data) {
  ...
  elements.rankLine.textContent = rankText(total);
...
function displayFeatureName(props) {
  ...
  if (props.municipality_name) {
    return `${props.municipality_name}の${layerLabel}`;
  }
  return layerLabel;
}
